...
const actors = document.getElementById('actorList');
...
//handle the movie list on window load
window.addEventListener('load', function(ev){
    data.movies.forEach(function(movie){
        let newA = document.createElement('a');
        newA.setAttribute('href','#');
        newA.textContent = movie.title;
        newA.addEventListener('click', poster);
        titleList.appendChild(newA);
    });
});//end load

//when the title is clicked post movie info
//calls displayM
function poster(event){
    data.movies.forEach(function(movie){
        if(event.target.textContent === movie.title){
            displayM(movie);
        }
    });
};//end poster

//display the movie content in the presentation area
function displayM(movie){
    title.textContent = movie.title;
    genre.textContent = movie.genres.join(' / ');
    actors.replaceChildren();
    let acts = movie.actors.split(', ');
    acts.forEach(function (name){
        let newAn = document.createElement('a');
        newAn.setAttribute('href','#');
        newAn.textContent = name;
        newAn.addEventListener('click', actorFilter);
        actors.appendChild(newAn);
    });
    let direc = movie.director.split(', ');
    director.replaceChildren();
    direc.forEach(function (nameD){
        let newAnc = document.createElement('a');
        newAnc.setAttribute('href','#');
        newAnc.textContent = nameD;
        newAnc.addEventListener('click', directFilter);
        director.appendChild(newAnc);
    });
...
//handle the year filters / buttons
buttList.addEventListener('click', function(ev){
    // console.dir(ev);
    if(ev.target.id != 'buttonList'){
        let firstMov;
        if(ev.target.textContent == 'ALL'){
            let titL = Array.from(titleList.getElementsByTagName('a'));
            titL.forEach(function (tit){
                tit.style.display = 'block';
                firstMov = data.movies[0];
            })
        }
        else{
            let range = ev.target.textContent.split('-');
            let rl = parseInt(range[0]);
            let rh = parseInt(range[1]);
            let movInR = [];
            data.movies.forEach(function (movie){
                if(parseInt(movie.year) >= rl && parseInt(movie.year) <= rh){
                    movInR.push(movie.title);
                    if(!firstMov){
                        firstMov = movie;
                    }
                }
            ...
            let titL = Array.from(titleList.getElementsByTagName('a'));
            titL.forEach(function (tit){
                if(movInR.includes(tit.textContent)){
                    tit.style.display = 'block';
                }
                else{
...
});//end filter buttons click buttList

//handle actors filter
function actorFilter(event){
    let act = event.target.textContent;
    let movAct = [];
    data.movies.forEach(function (movie){
        if(movie.actors.includes(act)){
            movAct.push(movie.title);
        }
    });
    console.dir(movAct);
    let titL = Array.from(titleList.getElementsByTagName('a'));
    console.dir(titL);
    titL.forEach(function(tit){
        if(movAct.includes(tit.textContent)){
            tit.style.display = 'block';
        }
        else{
            tit.style.display = 'none';
...
function directFilter(event){
        let direct = event.target.textContent;
        let movDirect = [];
        data.movies.forEach(function (movie){
            if(movie.director.includes(direct)){
                movDirect.push(movie.title);
            }
        });
        console.dir(movDirect);
        let titL = Array.from(titleList.getElementsByTagName('a'));
        console.dir(titL);
        titL.forEach(function(tit){
            if(movDirect.includes(tit.textContent)){
                tit.style.display = 'block';
            }
            else{
                tit.style.display = 'none';
...
function genreFilter(event){
  let genres = event.target.textContent;
  let movGenre = [];
  data.movies.forEach(function (movie){
      if(movie.genres.includes(genres)){
          movGenre.push(movie.title);
      }
  });
  let titG = Array.from(titleList.getElementsByTagName('a'));
  titG.forEach(function(gen){
      if(movDirect.includes(gen.textContent)){
